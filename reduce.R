...
reg_bruce <- loadRegistry("../lsm_applications/reg")
...
nodes <- 20L
res_open <- batchMapQuick(reduceResultsExperiments, chunk(findDone(reg_open), n.chunks = nodes, shuffle = TRUE),
...
res_open <- reduceResults(res_open, fun = function(aggr, job, res) rbind.fill(aggr, res), init = data.frame())

res_bruce <- batchMapQuick(reduceResultsExperiments, chunk(findDone(reg_bruce), n.chunks = nodes, shuffle = TRUE),
                          more.args = list(reg = reg_bruce, fun = reduce, progressbar = FALSE,
                                           impute.val = list("estimate" = NA, "loss" = NA, "coverage" = NA)),
                          resources = list(walltime = 86400L, nodes = 1L, memory = "8gb"))
res_bruce <- reduceResults(res_bruce, fun = function(aggr, job, res) rbind.fill(aggr, res), init = data.frame())
